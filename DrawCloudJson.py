import json
import os
output=open('DrawCloud.csv', 'w')
output.write('username,behance,artstation,facebook,google_plus,instagram,linkedin,pinterest,sketchfab,steam,tumblr,twitter,vimeo,youtube,dribbble,deviantart\n')

for file in os.listdir('/Users/YihanYu/Desktop/DrawCloud1000Users'):
    print file
    UserSocial = {}
    ...
    fileHandle = "/Users/YihanYu/Desktop/DrawCloud1000Users" + "//" + filename
    json_data= open(fileHandle).read()
    data = json.loads(json_data)
    username = data['user']['username']

    bio=data['user']['introduction']
    if bio != None:
        if 'behance' in bio:
    ...
    output.write(username+','+str(UserSocial['behance'])+","+str(UserSocial['artstation'])+","+str(UserSocial['facebook'])+","+str(UserSocial['google_plus'])+","+str(UserSocial['instagram'])+","+str(UserSocial['linkedin'])
                 + ","+str(UserSocial['pinterest'])+","+str(UserSocial['sketchfab'])+","+str(UserSocial['steam'])+","+str(UserSocial['tumblr'])+","+str(UserSocial['twitter'])+","+str(UserSocial['vimeo'])+","+str(UserSocial['youtube'])+","+str(UserSocial['dribbble'])+","+str(UserSocial['deviantart'])+'\n')
